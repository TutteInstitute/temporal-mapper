...
from holoviews.element.graphs import EdgePaths, Graph, Nodes
from holoviews.element.util import quadratic_bezier
...
class _layout_sankey(Operation):
    ...
    def computeNodeLinks(cls, element, graph):
        ...
        for i, (src, tgt, value) in enumerate(zip(*links)):
            weights = element.data["edge_weight"].iloc[i]
            source, target = node_map[src], node_map[tgt]
            link = dict(index=i, source=source, target=target, width0=weights[0], width1=weights[1])
    ...
    def computeNodeValues(cls, graph):
        ...
        """
        for node in graph['nodes']:
            #source_val = np.sum([l['width0'] for l in node['sourceLinks']])
            #target_val = np.sum([l['width1'] for l in node['targetLinks']])
    ...
    def initializeNodeBreadths(self, columns, py):
        ...
        for nodes in columns:
            ...
            for node in nodes:
                ...
                y = node['y1'] + py
                for link in node['sourceLinks']:
                    link['width0'] = link['width0'] * ky
                    link['width1'] = link['width1'] * ky
    ...
    def relaxLeftToRight(self, columns, alpha, beta, py):
        for column in columns[1:]:
            for target in column:
                ...
                for link in target['targetLinks']:
                    source = link['source']
                    v = link['width0'] * (target['column'] - source['column'])
    ...
    def computeLinkBreadths(cls, graph):
        ...
        for node in graph['nodes']:
            ...
            y1 = y0
            for link in node['sourceLinks']:
                link['y0'] = y0 + link['width0'] / 2
                y0 += link['width0']
            for link in node['targetLinks']:
                link['y1'] = y1 + link['width1'] / 2
                y1 += link['width1']
    ...
    def computePaths(self, graph):
        ...
        for link in graph['links']:
            ...
            x1 = target['x0']
            xmid = (x0 + x1) / 2
            y0_upper = link['y0'] + link['width0'] / 2
            y0_lower = link['y0'] - link['width0'] / 2
            y1_upper = link['y1'] + link['width1'] / 2
            y1_lower = link['y1'] - link['width1'] / 2
...
class Sankey(Graph):
    ...
    def __init__(self, data, kdims=None, vdims=None, **params):
        print("Initialized modified Sankey.")
        if data is None:
            data = []
        ...
        if compute:
            ...
            # Other parts of the library expect the "Value"
            # column to contain a scalar value so create a
            # new column to store the edge weights and use
            # the mean as the scalar value.
            self.data["edge_weight"] = self.data["Value"]
            self.data["Value"] = [(w[0]+w[1])/2 for w in self.data["Value"]]
